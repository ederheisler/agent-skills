...
**Why It Happens**: When you return a custom `Response` with a `background` parameter, it overwrites all tasks added to the injected `BackgroundTasks` dependency.
...
**Why It Happens**: Using Pydantic's `Json` type directly with `Form()` fails.
...
**Why It Happens**: Major breaking change when migrating from Pydantic v1 to v2.
...
**Why It Happens**: When raising `ValueError` inside a Pydantic `@field_validator` with Form fields, FastAPI returns 500 instead of 422.
