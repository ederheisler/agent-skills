...
Explain that this approach helps ensure the doc works well when others read it (including when they paste it into an AI agent). Ask if they want to try this workflow or prefer to work freeform.
...
- If images exist without alt-text, explain that when others use an AI agent to understand the doc, the agent won't be able to see them. Ask if they want alt-text generated. If so, request they paste each image into chat for descriptive alt-text generation.
...
**If no integrations are detected:** Suggest they can enable connectors (if supported by their AI platform) to allow pulling context from messaging apps and document storage directly.
...
- If user mentions team channels or shared documents:
  ...
  - If integrations not available: Explain lack of access. Suggest they enable connectors (if supported), or paste the relevant content directly.
...
### Step 1: Predict Reader Questions (Manual Testing)

Ask what questions people might ask when trying to discover this document. What would they ask an AI agent?
...
1. Open a fresh AI agent conversation
