...
from .. import plugin
from ..config import DESTINATION, LOG_FILE, logger
...
class SkillListScreen(Screen):
    ...
    def __init__(self):
        ...
        self.available_skills = load_skills()
    ...
    def _get_footer_left(self) -> str:
        return "Space: Toggle  U: Update All Installed  Enter: Apply  E: Description  L: Logs  Q: Quit"
    ...
    def action_reinstall_all(self) -> None:
            ...
            if skill.dir_name in installed_names:
                try:
                    if skill.is_plugin:
                        ...
                        plugin.remove_plugin(skill)
                        plugin.install_plugin(skill)
                        plugin_success_count += 1
                    else:
                        logger.info(f"Reinstalling skill {skill.name}")
                        if install_skill(skill, DESTINATION):
        ...
        if errors:
            ...
            if skill_success_count > 0:
                parts.append(f"{skill_success_count} skill{'s' if skill_success_count != 1 else ''}")
            if plugin_success_count > 0:
                parts.append(f"{plugin_success_count} plugin{'s' if plugin_success_count != 1 else ''}")
            msg = f"Successfully updated {' + '.join(parts)}" if parts else "No updates needed (or installed items not found in source)"
            self.notify(
                msg,
    ...
    def action_execute_install(self) -> None:
            ...
            try:
                if skill.is_plugin:
                    ...
                    if should_remove:
                        ...
                        if remove_skill(skill, DESTINATION):
                            success_count += 1
                        else:
        ...
        if errors:
            ...
            self.notify(f"Success! Processed {success_count} items.", severity="information")
